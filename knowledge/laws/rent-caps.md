...
## At a glance: four different dimensions, don't merge them into one rule

Added 2026-08-12, prompted by ChatGPT's second-pass Pre-Publish Audit of post 8842, which pointed out that this file's post-8842 corrections (rate, frequency, first-increase eligibility, and banking) are four genuinely separate questions that are easy to accidentally collapse into a single "annual rent increase rule" — which is exactly the mistake this file's earlier drafts made twice this session (once on AB 1482 frequency, once on the new-tenancy-delay comparison). This table is a quick-reference index into the fuller sections below, not a replacement for them — always check the relevant section for sourcing and caveats before citing a cell from this table directly.

| Jurisdiction | Annual cap | Frequency | First-increase eligibility | Banking |
|---|---|---|---|---|
...
### AB 1482 allows up to two increases per 12 months — not one

Caught by ChatGPT's Pre-Publish Audit of post 8842 (2026-08-12) and independently verified against two legal-code mirror sites the same day (`leginfo.legislature.ca.gov` returned `ROBOTS_DISALLOWED` for this specific section on this attempt — `codes.findlaw.com/ca/civil-code/civ-sect-1947-12/` and `law.justia.com/codes/california/code-civ/division-3/part-4/title-5/chapter-2/section-1947-12/` were used instead, and corroborate each other verbatim). Civil Code §1947.12(a)(2): "the gross rental rate for the unit of residential real property shall not be increased in more than two increments over that 12-month period." A landlord isn't limited to one AB 1482 increase per rolling 12 months — up to two separate increases are allowed, as long as their combined total stays within that period's cap. **Confidence: Level 1 (required by law) — confirmed 2026-08-12.** Post 8842's earlier draft stated a blanket "once per rolling 12 months" rule and applied it to all four jurisdictions; that's wrong for AB 1482 specifically (see the Oakland section below for where "once per 12 months" actually is the correct, stricter local rule).
...
### Oakland's own frequency and new-tenancy rules — not previously captured in this file

Not previously spelled out here, surfaced 2026-08-12 while fact-checking ChatGPT's Pre-Publish Audit of post 8842 (that review flagged Richmond's new-tenancy delay; checking it turned up that Oakland has one too, undisclosed until now). Confirmed by direct fetch of the same City of Oakland "Learn More About Allowable Rent Increases" page cited above, two additional direct quotes: *"An owner can increase the rent on a covered unit only once in a 12-month period"* and *"The first increase cannot be effective any earlier than 12 months after the tenant moved into the unit."* **Confidence: Level 1 (required by local ordinance, City-published) — confirmed 2026-08-12.** Two distinct rules worth keeping separate: (1) a strict once-per-12-months frequency limit — stricter than AB 1482's "up to two increments" rule above, no exceptions even if a second increase would stay under the combined 2.3% cap; (2) a new-tenancy delay — the very first increase on a unit can't take effect until 12 months after the tenant's actual move-in date. This means Oakland is not the only local jurisdiction with a new-tenancy delay alongside Berkeley — see the correction in the Berkeley section below, and Richmond's own version in `richmond-rent-program.md`.
...
Not previously spelled out in this file with the actual rule text — only implied above by the vague "applies to tenancies starting before Jan 1 of the prior year" phrasing, which undersells how unusual this rule actually is. Flagged by Brian 2026-08-12, verified against the Berkeley Rent Board's own AGA page the same day (`rentboard.berkeleyca.gov/rights-responsibilities/rent-levels/annual-general-adjustment`), direct quote:
...
**Confidence: Level 1 (required by local ordinance, City-published) — confirmed 2026-08-12.** This is a restriction on *when a unit first becomes AGA-eligible at all*, separate from and in addition to the 1.0% rate itself — a brand-new Berkeley tenancy is AGA-increase-ineligible for close to two full calendar years regardless of the rate. The rule is keyed to the tenancy's start date (when the tenant took occupancy), not the lease type or any conversion date — relevant for a tenancy that starts as a fixed term and later converts to month-to-month under Civil Code §1945, since the clock still runs from the original move-in date, not the conversion date.

**Correction, 2026-08-12 (this paragraph was wrong in the version committed the same day, caught same-session while fact-checking ChatGPT's Pre-Publish Audit of post 8842): Berkeley is NOT the only jurisdiction with a new-tenancy delay.** Oakland has its own (12 months from the tenant's move-in date — see the Oakland section above, confirmed the same day directly against oaklandca.gov) and Richmond has its own (a tenancy must exist a full calendar year before the next September 1 AGA date — see `richmond-rent-program.md`, confirmed the same day against a City of Richmond presentation PDF). AB 1482 is the one genuine outlier, and for a different reason than "no delay" — the statewide cap only restricts increases on a tenant already in occupancy, and the very first rent charged to a new tenant isn't regulated at all, so there's no comparable delay concept to apply. All three local jurisdictions (Oakland, Berkeley, Richmond) delay a brand-new tenancy's first increase; they just use three different clocks. Berkeley's is the longest of the three (rest of the start year plus one full additional calendar year), not the only one.
...
**2026-08-12 fact-check pass (prompted by ChatGPT's Pre-Publish Audit of post 8842):** two corrections landed in this file the same day. (1) AB 1482 allows up to two increases per 12 months, not one — added a new subsection under the AB 1482 table, confirmed against two legal-code mirrors after `leginfo.legislature.ca.gov` returned `ROBOTS_DISALLOWED` for this specific section on this attempt. (2) The Berkeley new-tenancy-delay subsection (added 2026-08-12 earlier the same session) incorrectly claimed Berkeley was the only one of AEBP's jurisdictions with a new-tenancy delay — Oakland has its own (added as a new subsection under the Oakland table, confirmed against oaklandca.gov) and Richmond has its own (documented in `richmond-rent-program.md`, confirmed against a City of Richmond presentation PDF). Corrected in place rather than left standing, since the original version had already been committed to the repo (commit `d2b3c64`) before the error was caught.
