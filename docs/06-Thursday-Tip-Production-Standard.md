...
- **Series label + date:** e.g. "Owner ROI Series · July 23, 2026." **There is no formal list of series names** — known ones so far are "Owner ROI Series," "Move-Out Series," and "Renewal Series" (confirmed 2026-07-30, used for all four August 2026 Thursday Tips, which are renewal-adjacent — pairs with "Renewal Season" as the broader August campaign/homepage name; the badge itself stays "Renewal Series" to match the fixed "[Topic] Series" pattern). Ask Brian for the right one each week rather than inventing a new one; add it to this list once confirmed. **"Rent Control Series" confirmed 2026-09-03** for September 2026's city-specific rent control deep-dive month — matches the eyebrow text on that month's hero graphic, used starting with the Week 1 Thursday Tip.
...
- 2026-09-03: Added "Rent Control Series" to the known series-label list, confirmed by Brian for September 2026 (matches the month's hero graphic eyebrow). A first draft of the Week 1 Thursday Tip had proposed an unconfirmed "Compliance Series" label instead, since this repo wasn't connected to that session yet — corrected once Brian connected it and pointed to the real hero graphic.
...
- 2026-08-05: Found a real rendering bug in `MC-Tip-Email-Template.html`'s section-icon markup (the 🧠/💡/📘 badges in `.section-label`), confirmed by Brian via a Gmail-web screenshot: the `<span class="section-icon-wrap">...emoji...</span>` pattern (inline-block + `line-height` for vertical centering) renders fine in Outlook but drops the emoji glyph well below its rounded badge in Gmail's web client — emoji glyphs don't reliably obey CSS `line-height` on inline-block spans across renderers. Fix: replace each icon span with a one-cell `<table role="presentation">`/`<td width="28" height="28" valign="middle">` construction instead — table-cell `valign` is centered far more consistently than span `line-height` for emoji across Outlook/Gmail/Yahoo/Apple Mail. Applied to the Week 1 August 2026 ("Renewal Series") email. **Any future Thursday Tip email built from this template should use the table-cell icon pattern, not the original span pattern** — the template file itself still has the old (buggy) markup and should be corrected next time it's touched, since this file describes the template's edit points but isn't the template file itself.
